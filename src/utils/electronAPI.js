...
export const dialog = {
    ...
    async showOpenDialogItemName(options = {}) {
        if (!isElectron()) {
            throw new Error("Dialog API 只在 Electron 环境下可用");
        }
        const defaultOptions = {
        ...
        return await window.electronAPI.showOpenDialogItemName({
            ...defaultOptions,
            ...options,
        });
    },
...
export const videOperate = {
    async compressVideo(fullPath) {
        if (!isElectron()) {
            throw new Error("视频压缩只在 Electron 环境下可用");
        }

        return await window.electronAPI.compressVideo(fullPath);
    },
};
