...
class DownloadService {
    ...
    async downloadChannelContent(config) {
        ...
        if (this.useMultiThreadDownload) {
            ...
            multiThreadDownloadManager.addProgressCallback(
                multiThreadProgressCallback
            );
        ...
        try {
            ...
            while (hasMoreMessages && this.isDownloading) {
                try {
                    console.log(`\n📦 ===== 批次 ${batchNumber} 开始 =====`);
                    console.log(
                        `� 获取消息批次，offsetId: ${currentOffsetId}, 批次大小: ${batchSize}`
                    );
                    ...
                    const batchMessages =
                        await telegramService.client.getMessages(
                            dialog.entity,
                            batchOptions
                        );
                    ...
                    const filteredBatch = batchMessages.filter((msg) => {
                        if (startMessageId && msg.id < startMessageId)
                            return false;
                    ...
                    console.log(
                        `📨 批次获取到 ${batchMessages.length} 条消息，过滤后 ${filteredBatch.length} 条`
                    );
                    ...
                    if (filteredBatch.length === 0) {
                        ...
                        if (batchMessages.length > 0) {
                            ...
                            console.log(
                                `📄 批次无匹配消息，更新 offsetId 为: ${currentOffsetId}`
                            );
                        } else {
                            // 没有更多消息了
                            hasMoreMessages = false;
                    ...
                    for (let i = 0; i < filteredBatch.length; i++) {
                        ...
                        const progressUpdate = {
                            ...
                            status: `正在处理第 ${batchNumber} 批第 ${i + 1}/${
                        ...
                        try {
                            // 检查消息是否符合过滤条件
                            const shouldInclude = this.shouldDownloadFile(
                                message,
                                filenameFilter,
                                filterMode,
                                minFileSize,
                            ...
                            if (
                                message.media &&
                                this.shouldDownloadMedia(
                                    message.media,
                                    downloadTypes
                                )
                            ) {
                                console.log(
                                    `🎬 消息 ${message.id} 媒体类型检查通过，准备下载检查`
                                );
                                ...
                                if (shouldInclude) {
                                    ...
                                    if (message.groupedId) {
                                        const groupedIdStr = String(
                                            message.groupedId
                                        );

                                        // 检查是否已经处理过这个媒体组
                                        if (
                                            processedGroupIds.has(groupedIdStr)
                                        ) {
                                            console.log(
                                                `📱 媒体组 ${groupedIdStr} 已处理，跳过消息 ${message.id}`
                                            );
                                        } else {
                                            console.log(
                                                `📱 消息 ${message.id} 是媒体组的一部分 (groupedId: ${message.groupedId})`
                                            );
                                            // 获取同一媒体组的所有消息
                                            const groupMessages =
                                                await this.getGroupedMessages(
                                                    dialog.entity,
                                                    message.groupedId,
                                                    filteredBatch
                                                );
                                            ...
                                            if (this.useMultiThreadDownload) {
                                                ...
                                                if (messageInfo) {
                                                    ...
                                                    messageInfo.media.groupedId =
                                                        message.groupedId;
                                                    messageInfo.media.downloadedAt =
                                                        new Date().toISOString();
                                                }
                                            } else {
                                                // 使用原有的同步下载方式
                                                const downloadResults =
                                                    await this.downloadMediaGroup(
                                                        groupMessages,
                                                        channelDir,
                                                        downloadTypes,
                                                        onProgress
                                                ...
                                                if (
                                                    ...
                                                    messageInfo.media.downloadPaths =
                                                        downloadResults.map(
                                                            (result) =>
                                                                result.filePath
                                                        );
                                        ...
                                        if (this.useMultiThreadDownload) {
                                            ...
                                            if (
                                                ...
                                                messageInfo.media.downloadPath =
                                                    downloadResult.filePath;
                                                messageInfo.media.downloadedAt =
                                                    new Date().toISOString();
                                                messageInfo.media.fileExists = true;
                    ...
                    console.log(
                        `📄 更新 offsetId 为: ${currentOffsetId}，准备获取更早的消息`
                    );
            ...
            if (this.useMultiThreadDownload) {
                ...
                while (
                    multiThreadDownloadManager.getStats().activeDownloads > 0 &&
                        ...
                        if (messageInfo.media.isMediaGroup) {
                            ...
                            if (groupResults.length > 0) {
                                messageInfo.media.downloadPaths = groupResults
                                    ...
                                    .map((r) => r.filePath);
                                messageInfo.media.fileExists =
                                    groupResults.some((r) => r.success);
                            }
                        } else {
                            // 单个文件：查找单个下载结果
                ...
                console.log(
                    `✅ 多线程下载统计: 完成 ${finalStats.completed}, 失败 ${finalStats.failed}`
                );
            }

            // 分文件保存消息数据
            await this.saveMessagesJsonByChunks(
                allMessageData,
                `${channelDir}/json`
            );
            ...
            if (progressData.type === "overall") {
                ...
                onProgress({
                    ...
                    multiThread: {
                        ...
                        totalDownloaded: this.formatFileSize(
                ...
                // 文件下载失败
                console.error(
                    `❌ 多线程下载失败: ${progressData.fileName}, 错误: ${progressData.error}`
                );
            }
        } catch (error) {
    ...
    extractMessageData(message) {
        // 处理BigInt类型的ID
        const messageId =
            typeof message.id === "bigint" ? Number(message.id) : message.id;
    ...
    getMediaType(media) {
        ...
        if (
            media.video ||
            (media._ === "messageMediaDocument" && media.document?.videoSizes)
        )
            return "video";
        if (media.document || media._ === "messageMediaDocument")
            return "document";
        return "other";
    }
    /**
    ...
    shouldDownloadFile(
        ...
        if (filenameFilter && filenameFilter.trim() !== "") {
            // 支持多个关键字，使用 | 分隔
            const filterKeywords = filenameFilter
                .toLowerCase()
                .trim()
                .split("|")
                .map((k) => k.trim())
            ...
            if (message.message) {
                ...
                matchFound = filterKeywords.some((keyword) =>
            ...
            console.log(`🔍 文件名过滤结果:`, {
                ...
                messageText: message.message
                    ...
                    : "none",
                fileName:
                    message.media && message.media.document
                        ? message.media.document.attributes?.[0]?.fileName ||
        ...
        if (
            (minFileSize !== null && minFileSize > 0) ||
            ...
            console.log("📏 文件大小检查:", {
                ...
                mediaType: message.media
                    ? this.getMediaType(message.media)
                    : "none",
            });
            ...
            if (
                minFileSize !== null &&
                minFileSize > 0 &&
                fileSizeKB < minFileSize
            ) {
                sizeMatched = false;
                console.log(
                    `❌ 文件太小: ${fileSizeKB.toFixed(
                        2
                    )} KB < ${minFileSize} KB`
                );
            }
            ...
            if (
                maxFileSize !== null &&
                maxFileSize > 0 &&
                fileSizeKB > maxFileSize
            ) {
                sizeMatched = false;
                console.log(
                    `❌ 文件太大: ${fileSizeKB.toFixed(
                        2
                    )} KB > ${maxFileSize} KB`
                );
            }
    ...
    getMediaFileName(media, messageId, groupIndex = null) {
        ...
        if (media.document) {
            ...
            if (originalFileName) {
                ...
                if (groupIndex) {
                    const fileExtension =
                        this.getExtensionFromFileName(sanitizedName);
                    if (fileExtension) {
                ...
                if (hasValidExtension) {
                    ...
                    const mimeExtension = this.getExtensionFromMimeType(
                        doc.mimeType
                    );
                    ...
                    if (mimeExtension && mimeExtension !== "bin") {
                        ...
                        const inferredExt = this.inferExtensionFromContent(
                            sanitizedName,
                            doc.mimeType
                        );
                        return inferredExt
                            ? `${sanitizedName}.${inferredExt}`
                            : sanitizedName;
                    }
                }
            }
    ...
    async downloadMediaFile(
        ...
        try {
            ...
            const mediaType = this.getMediaType(media);
            const fileName = this.getMediaFileName(
                media,
                message.id,
                groupIndex
            );
            ...
            const buffer = await telegramService.client.downloadMedia(message, {
                progressCallback: (downloaded, total) => {
                    ...
                    console.log(
                        `📥 下载进度 ${fileName}: ${percent.toFixed(1)}%`
                    );
                    ...
                    if (onProgress) {
                        onProgress({
                            ...
                            status: `正在下载文件: ${fileName} (${percent.toFixed(
                                1
                            )}%)`,
                        });
                    }
    ...
    getChannelDownloadRecord(channelId) {
        try {
            ...
            if (
                ...
                parsed !== null &&
                Array.isArray(parsed.downloadSessions)
            ) {
                return parsed;
            } else {
    ...
    async saveMessagesJsonByChunks(messageData, baseDir) {
        try {
            ...
            for (let i = 0; i < totalChunks; i++) {
                ...
                console.log(
                    `💾 已保存消息文件: ${fileName} (${chunk.length} 条消息)`
                );
            }
    ...
    async getGroupedMessages(entity, groupedId, currentBatch = []) {
        try {
            ...
            const groupMessages = currentBatch.filter(
                (msg) =>
                    msg.groupedId &&
                    ((typeof msg.groupedId === "bigint" &&
                        typeof groupedId === "bigint" &&
                        msg.groupedId === groupedId) ||
                        (typeof msg.groupedId === "number" &&
            ...
            console.log(
                `📱 在当前批次中找到 ${groupMessages.length} 条媒体组消息`
            );
    ...
    async downloadMediaGroup(
        ...
        try {
            console.log(
                `📱 开始下载媒体组，包含 ${groupMessages.length} 个媒体文件`
            );
            ...
            console.log(
                `✅ 媒体组下载完成，成功下载 ${downloadResults.length} 个文件`
            );
