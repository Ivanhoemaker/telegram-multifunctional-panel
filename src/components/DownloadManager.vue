                ...
                <v-col cols="12" md="5" lg="4" xl="4" class="sidebar-col">
                    <v-card class="sidebar-card h-100" elevation="0" rounded="xl">
                                ...
                                <div class="d-flex align-center">
                                    ...
                                    <div>
                                        <h3 class="header-title">
                                            {{ $t("download.dialogList") }}
                                        </h3>
                                        <p class="header-subtitle">
                                            {{
                                                $t(
                                                    "download.selectDialogPrompt"
                                                )
                                            }}
                                        </p>
                                    </div>
                        ...
                        <div class="dialog-filter pa-4">
                            <v-text-field v-model="dialogSearchText" :placeholder="$t('download.searchDialogPlaceholder')
                                " prepend-inner-icon="mdi-magnify" variant="outlined" density="compact" clearable
                                hide-details class="mb-3" @clear="dialogSearchText = ''">
                                        ...
                                        <v-card min-width="240" class="filter-menu">
                                            <v-card-text class="pb-2">
                                                <div class="text-subtitle-2 mb-2">
                                                    {{
                                                        $t(
                                                            "download.filterByType"
                                                        )
                                                    }}
                                                </div>
                                                <v-checkbox v-model="dialogTypeFilters.channels
                                                    " :label="$t('download.channels')
                                                        " density="compact" hide-details class="mb-1"></v-checkbox>
                                                <v-checkbox v-model="dialogTypeFilters.groups
                                                    " :label="$t('download.groups')
                                                        " density="compact" hide-details class="mb-1"></v-checkbox>
                                                <v-checkbox v-model="dialogTypeFilters.users
                                                    " :label="$t('download.users')
                                                        " density="compact" hide-details></v-checkbox>
                                            </v-card-text>
                                            <v-card-actions class="pt-0">
                                                <v-btn variant="text" size="small" @click="resetDialogFilters">
                                                    {{ $t("common.reset") }}
                                                </v-btn>
                                            </v-card-actions>
                                        </v-card>
                            ...
                            <!-- 筛选结果统计 -->
                            <div v-if="
                                dialogSearchText || !allDialogTypesSelected
                            " class="filter-info">
                                ...
                                <template v-if="loadingDialogs">
                                    <v-list-item v-for="i in 6" :key="i" class="skeleton-item">
                                        <v-skeleton-loader type="list-item-avatar-two-line"
                                            class="transparent-skeleton"></v-skeleton-loader>
                                    </v-list-item>
                                </template>

                                <!-- 对话项 -->
                                <template v-else>
                                    <v-list-item v-for="dialog in filteredDialogs" :key="dialog.id" :class="{
                                        'selected-dialog':
                                            selectedDialog?.id ===
                                            dialog.id,
                                        'dialog-item': true,
                                    }" @click="selectDialog(dialog)" class="modern-dialog-item" rounded="lg">
                                        <template v-slot:prepend>
                                            <v-avatar :color="getDialogColor(dialog)" class="dialog-avatar" size="48">
                                                <v-icon :icon="getDialogIcon(dialog)
                                                    " color="white" size="24"></v-icon>
                                            </v-avatar>
                                        </template>
                                        ...
                                        <div class="dialog-content">
                                            ...
                                            <v-list-item-subtitle class="dialog-meta">
                                                <v-chip :color="getTypeColor(dialog)
                                                    " size="x-small" class="type-chip me-2" variant="tonal">
                                    ...
                                    <p class="empty-text">
                                        {{
                                            dialogs.length === 0
                                                ? $t("common.noData")
                                                : $t(
                                                    "download.noMatchingDialogs"
                                                )
                                        }}
                                    </p>
                                    <v-btn v-if="dialogs.length === 0" color="primary" variant="tonal"
                                        @click="refreshDialogs" size="small">
                                        {{ $t("common.refresh") }}
                                    </v-btn>
                                    <v-btn v-else color="primary" variant="tonal" @click="resetDialogFilters"
                                        size="small">
                                        {{ $t("common.reset") }}
                                    </v-btn>
                                </div>
                    ...
                    <v-card class="main-card h-100" elevation="0" rounded="xl">
                        ...
                        <div class="main-header">
                            <div class="header-content">
                                ...
                                <div class="d-flex align-center gap-2">
                                    <v-chip v-if="selectedDialog" color="white" variant="elevated" size="small"
                                        class="selected-chip">
                                        <v-icon start size="16">{{
                                            getDialogIcon(selectedDialog)
                                            }}</v-icon>
                                        {{
                        ...
                        <v-card-text class="main-content pa-8">
                            <v-form v-model="formValid" @submit.prevent="startDownload">
                                <!-- 选中的对话信息 -->
                                <v-expand-transition>
                                    <v-alert v-if="selectedDialog" type="info" variant="tonal" icon=""
                                        class="mb-8 selected-alert" rounded="xl">
                                        <div class="d-flex align-center">
                                            <v-avatar :color="getDialogColor(
                                                selectedDialog
                                            )
                                                " size="40" class="me-4">
                                                <v-icon :icon="getDialogIcon(
                                                    selectedDialog
                                                )
                                                    " color="white"></v-icon>
                                            </v-avatar>
                                ...
                                <v-expand-transition>
                                    ...
                                    " type="success" variant="tonal" class="mb-8 download-record-alert" icon=""
                                        rounded="xl">
                                                ...
                                                <div class="stat-row">
                                                    <div class="stat-item">
                                                        ...
                                                        <span class="stat-label">{{
                                                            $t(
                                                                "download.record.sessions"
                                                            )
                                                        }}：</span>
                                                        <span class="stat-value">{{
                                                            downloadRecordSummary.totalSessions
                                                        }}</span>
                                                    </div>
                                                    <div class="stat-item">
                                                        <v-icon size="16" class="me-2"
                                                            color="success">mdi-file-check</v-icon>
                                                        <span class="stat-label">{{
                                                            $t(
                                                        ...
                                                        <span class="stat-value">{{
                                                            downloadRecordSummary.totalDownloaded
                                                        }}</span>
                                                    </div>
                                                </div>

                                                <div class="stat-row" v-if="
                                                    downloadRecordSummary.messageRange
                                                ">
                                                    <div class="stat-item">
                                                        <v-icon size="16" class="me-2"
                                                            color="info">mdi-message-text</v-icon>
                                                        <span class="stat-label">{{
                                                            $t(
                                                                "download.record.messageRange"
                                                            )
                                                        }}：</span>
                                                        <span class="stat-value">
                                                            {{
                                                    ...
                                                    <div class="stat-item">
                                                        <v-icon size="16" class="me-2"
                                                            color="warning">mdi-clock-outline</v-icon>
                                                        <span class="stat-label">{{
                                                            $t(
                                                                "download.record.lastDownload"
                                                            )
                                                        }}：</span>
                                                        <span class="stat-value">
                                                            {{
                                                                new Date(
                                                                    downloadRecordSummary.lastDownload
                                                                ).toLocaleDateString()
                                                            }}
                                                        </span>
                                                    </div>
                                ...
                                <v-expand-transition>
                                    <v-alert v-if="!selectedDialog" type="warning" variant="tonal"
                                        class="mb-8 selection-prompt" icon="" rounded="xl">
                                        <div class="d-flex align-center">
                                            <v-icon class="me-3" size="32">mdi-arrow-left-circle-outline</v-icon>
                                            <div>
                                                <div class="font-weight-medium text-h6 mb-1">
                                                    {{
                                                        $t(
                                                            "download.selectDialog"
                                                        )
                                                    }}
                                                </div>
                                ...
                                <!-- 配置表单 -->
                                <div class="config-form" :class="{
                                    'form-disabled': !selectedDialog,
                                }">
                                    <!-- 下载类型选择 -->
                                    <div class="form-section mb-8">
                                        ...
                                        <v-select v-model="downloadTypes" :items="downloadTypeOptions"
                                            :label="$t('download.selectTypes')" multiple variant="solo"
                                            bg-color="surface-variant" :rules="[
                                                (v) =>
                                                    v.length > 0 ||
                                                    $t(
                                                        'download.errors.typesRequired'
                                                    ),
                                            ]" class="modern-select" rounded="xl" hide-details="auto">
                                            <template v-slot:selection="{
                                                item,
                                                index,
                                            }">
                                                <v-chip v-if="index < 3" :key="item.value" size="small" class="ma-1"
                                                    closable @click:close="
                                                        removeDownloadType(
                                                            item.value
                                                        )
                                                        " :color="item.raw.color" variant="tonal">
                                    ...
                                    <div class="form-section mb-8">
                                        ...
                                        <v-expand-transition>
                                            <v-alert v-if="
                                                ...
                                                downloadRecordSummary &&
                                                downloadRecordSummary.messageRange
                                            " type="info" variant="tonal" class="mb-4 downloaded-range-alert"
                                                rounded="lg">
                                                <div class="d-flex align-center">
                                                    ...
                                                    <div class="flex-grow-1">
                                                        <div class="font-weight-medium">
                                                            {{
                                                                $t(
                                                                    "download.record.downloadedRange"
                                                                )
                                                            }}
                                                        </div>
                                        ...
                                        <v-row>
                                            <v-col cols="12" sm="6">
                                                <v-text-field v-model="startMessageId" :label="$t(
                                                    'download.startMessageId'
                                                )
                                                    " type="number" variant="solo" bg-color="surface-variant"
                                                    :placeholder="$t(
                                                        'download.startMessageIdPlaceholder'
                                                    )
                                                        " class="modern-input" rounded="xl" hide-details="auto">
                                            ...
                                            <v-col cols="12" sm="6">
                                                <v-text-field v-model="endMessageId" :label="$t(
                                                    'download.endMessageId'
                                                )
                                                    " type="number" variant="solo" bg-color="surface-variant"
                                                    :placeholder="$t(
                                                        'download.endMessageIdPlaceholder'
                                                    )
                                                        " class="modern-input" rounded="xl" hide-details="auto">
                                                    <template v-slot:prepend-inner>
                                    ...
                                    <div class="form-section mb-8">
                                        ...
                                        <v-text-field v-model="filenameFilter" :label="$t(
                                            'download.filenameFilterLabel'
                                        )
                                            " variant="solo" bg-color="surface-variant"
                                            :placeholder="'例如: 视频|电影|music 或 .jpg|.png'" class="modern-input"
                                            rounded="xl" hide-details="auto" clearable>
                                            <template v-slot:prepend-inner>
                                        ...
                                        <div v-if="
                                            filenameFilter &&
                                            filenameFilter.trim()
                                        " class="mt-3 filter-keywords-section">
                                            <div class="d-flex flex-wrap align-center filter-keywords-chips">
                                                ...
                                                <v-chip v-for="(
keyword, index
                                                    ) in filenameFilterKeywords" :key="index" size="small" :color="filterMode === 'exclude'
                                                        ? 'error'
                                                        : 'success'
                                                ...
                                                <v-chip v-if="
                                                    filenameFilterKeywords.length >
                                                    1
                                                " size="small" color="info" variant="outlined" class="ma-1">
                                        ...
                                        <div v-if="
                                            ...
                                            filenameFilter.trim()
                                        " class="mt-4">
                                            <v-radio-group v-model="filterMode" inline hide-details
                                                class="filter-mode-group">
                                                <v-radio :label="$t(
                                                    'download.filterModeInclude'
                                                )
                                                    " value="include" color="success">
                                                ...
                                                <v-radio :label="$t(
                                                    'download.filterModeExclude'
                                                )
                                                    " value="exclude" color="error">
                                                    <template v-slot:label>
                                                        <div class="d-flex align-center">
                                                            {{
                                                                $t(
                                    ...
                                    <div class="form-section mb-8">
                                        ...
                                        <v-row>
                                            <v-col cols="12" sm="6">
                                                <v-text-field v-model.number="minFileSize" :label="$t(
                                                    'download.minFileSizeLabel'
                                                )
                                                    " variant="solo" bg-color="surface-variant" :placeholder="$t(
                                                        'download.minFileSizePlaceholder'
                                                    )
                                                        " class="modern-input" type="number" min="0" step="1"
                                                    rounded="xl" hide-details="auto" clearable suffix="KB">
                                                    <template v-slot:prepend-inner>
                                            ...
                                            <v-col cols="12" sm="6">
                                                <v-text-field v-model.number="maxFileSize" :label="$t(
                                                    'download.maxFileSizeLabel'
                                                )
                                                    " variant="solo" bg-color="surface-variant" :placeholder="$t(
                                                        'download.maxFileSizePlaceholder'
                                                    )
                                                        " class="modern-input" type="number" min="0" step="1"
                                                    rounded="xl" hide-details="auto" clearable suffix="KB">
                                                    <template v-slot:prepend-inner>
                                        ...
                                        <div class="mt-3">
                                            <v-alert type="info" variant="tonal" density="compact" rounded="lg"
                                                ...
                                                <div class="mt-2 text-caption opacity-75">
                                                    {{
                                                        $t(
                                                            "download.fileSizeFilterExample"
                                                        )
                                                    }}
                                                </div>
                                            </v-alert>
                                    ...
                                    <div class="form-section mb-8">
                                        ...
                                        <div class="download-settings">
                                            <v-switch v-model="useMultiThreadDownload" color="primary"
                                                class="multi-thread-switch" hide-details>
                                                <template v-slot:label>
                                                    <div class="d-flex align-center">
                                    ...
                                    <div class="form-section mb-8">
                                        ...
                                        <v-text-field v-model="downloadPath" :label="$t('download.selectPath')"
                                            variant="solo" bg-color="surface-variant" :rules="[
                                                (v) =>
                                                    !!v ||
                                                    $t(
                                                        'download.errors.pathRequired'
                                                    ),
                                            ]" class="modern-input path-input" readonly rounded="xl"
                                            hide-details="auto" @click="selectDownloadPath">
                                            <template v-slot:prepend-inner>
                                                <v-icon color="primary">mdi-folder</v-icon>
                                            </template>
                                            <template v-slot:append-inner>
                                                <v-btn icon variant="text" @click="selectDownloadPath" size="small">
                                    ...
                                    <v-expand-transition>
                                        <v-alert v-if="
                                            downloadPath && selectedDialog
                                        " type="info" variant="tonal" class="mb-8 path-info" rounded="xl">
                                            <div class="d-flex align-start">
                                                <div>
                                                    <div class="font-weight-medium mb-2">
                                                        {{
                                                            $t(
                                                                "download.structure.saveToPath"
                                                            )
                                                        }}：
                                                    </div>
                                                    ...
                                                    <div class="folder-structure">
                                                        ...
                                                        <div class="structure-item">
                                                            ...
                                                            <span>images/ -
                                                                {{
                                                                    $t(
                                                                        "download.structure.imagesFolder"
                                                                    )
                                                                }}</span>
                                                        </div>
                                                        <div class="structure-item">
                                                            ...
                                                            <span>videos/ -
                                                                {{
                                                                    $t(
                                                                        "download.structure.videosFolder"
                                                                    )
                                                                }}</span>
                                                        </div>
                                                        <div class="structure-item">
                                                            <v-icon size="16"
                                                                class="me-2">mdi-folder-text-outline</v-icon>
                                                            <span>documents/ -
                                                                {{
                                                                    $t(
                                                                        "download.structure.documentsFolder"
                                                                    )
                                                                }}</span>
                                                        </div>
                                                        <div class="structure-item">
                                                            ...
                                                            <span>others/ -
                                                                {{
                                                                    $t(
                                                                        "download.structure.othersFolder"
                                                                    )
                                                                }}</span>
                                                        </div>
                                                    </div>
                                    ...
                                    <div class="download-buttons-section">
                                        <v-btn type="submit" color="primary" size="x-large" :disabled="!selectedDialog ||
                                            !formValid ||
                                            isDownloading
                                            " :loading="isDownloading" class="download-btn flex-grow-1" rounded="xl">
                                        ...
                                        <v-btn color="orange" variant="tonal" size="x-large" :disabled="isDownloading"
                                            class="reset-btn ml-3" rounded="xl" @click="resetForm">
        ...
        <!-- 下载进度对话框 -->
        <v-dialog v-model="showProgressDialog" persistent width="600" class="progress-dialog">
            <v-card class="progress-card" rounded="xl" elevation="24">
                ...
                <v-card-text class="pa-8">
                    ...
                    <div class="progress-section mb-6">
                        <div class="d-flex justify-space-between align-center mb-3">
                            <span class="progress-label">{{
                                $t("download.progress.totalProgress")
                                }}</span>
                        ...
                        <v-progress-linear :model-value="downloadProgress.total > 0
                            ? (downloadProgress.current /
                                downloadProgress.total) *
                            100
                            : 0
                            " height="12" color="primary" rounded class="progress-bar"></v-progress-linear>
                    </div>
                    ...
                    <div class="status-section mb-6">
                        ...
                        <div v-if="
                            downloadProgress.currentFile &&
                            downloadProgress.fileProgress !== undefined
                        " class="file-progress-section">
                            <div class="d-flex justify-space-between align-center mb-2">
                                <span class="file-progress-label">{{
                                    $t("download.progress.fileProgress")
                                    }}</span>
                                <span class="file-progress-percent">{{
                                    downloadProgress.fileProgress.toFixed(
                                        1
                                    )
                                }}%</span>
                            </div>
                            <v-progress-linear :model-value="downloadProgress.fileProgress" height="8" color="secondary"
                                rounded class="file-progress-bar"></v-progress-linear>
                            ...
                            <div v-if="downloadProgress.taskProgress" class="mt-2 text-caption">
                                <div class="d-flex justify-space-between">
                                    <span>{{
                                        ...
                                        /
                                        {{
                                            downloadProgress.taskProgress.total
                                        }}</span>
                                    <span>{{
                                        downloadProgress.taskProgress.speed
                                        }}</span>
                                </div>
                            </div>
                        </div>
                ...
                <v-card-actions class="pa-6 pt-0">
                    <v-spacer></v-spacer>
                    <v-btn @click="cancelDownload" :color="cancelButtonProps.color" variant="tonal" size="large"
                        rounded="lg">
...
const allDialogTypesSelected = computed(() => {
    return (
        dialogTypeFilters.value.channels &&
...
const downloadRecordSummary = computed(() => {
    ...
    const totalDownloaded = sessions.reduce(
        (sum, session) => sum + session.downloaded,
        0
    );
...
const isDownloadCompleted = computed(() => {
    return (
        ...
        (downloadProgress.value.status.includes(
            t("download.progress.completed")
        ) ||
            downloadProgress.value.status.includes(
                t("download.progress.failed")
            ))
    );
});
...
async function startDownload() {
    ...
    try {
        ...
        const config = {
            ...
            startMessageId: startMessageId.value
                ? parseInt(startMessageId.value)
                : null,
            endMessageId: endMessageId.value
                ? parseInt(endMessageId.value)
                : null,
        ...
        // 显示完成消息
        downloadProgress.value.status = `${t(
            "download.progress.completed"
        )}！${t("common.total")}${t("download.progress.processed")} ${result.totalMessages
            } ${t("download.progress.messages")}，${t(
                "download.progress.downloaded"
            )} ${result.downloaded} ${t("download.record.fileCount")}`;
...
.download-btn {
    ...
    background: linear-gradient(135deg,
            rgb(var(--v-theme-primary)) 0%,
            rgb(var(--v-theme-primary-darken-1)) 100%) !important;
