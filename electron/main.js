...
const fs = require("fs");
const { getVideoInfo, extractThumbnailBuffer } = require("./ffmpeg");
// 设置开发环境
...
function createWindow() {
    ...
    if (VITE_DEV_SERVER_URL) {
        ...
        if (fs.existsSync(indexPath)) {
            ...
            // 显示错误页面
            win.loadURL(
                `data:text/html,<h1>应用启动失败</h1><p>找不到应用文件: ${indexPath}</p>`
            );
        }
    }
...
//  ---                ---------------------------------
// 选择文件夹并且获取文件夹内的视频文件信息
ipcMain.handle("show-open-dialog-item-name", async (event) => {
    try {
        const result = await dialog.showOpenDialog(win, {
            title: "选择文件夹",
            defaultPath: app.getPath("downloads"),
        ...
        // 如果用户选择了文件夹，读取文件夹内的文件名
        if (
            result.filePaths &&
            result.filePaths.length > 0 &&
            !result.canceled
        ) {
            const folderPath = result.filePaths[0]; // 选中的文件夹路径
            try {
                // 读取文件夹下的所有文件名,只读视频文件
                ...
                const videoPromises = videoFiles.map(async (file) => {
                    ...
                    const fileName = path.basename(videoInfo.format.filename);
                    const fullPath = path.join(folderPath, file);
                    const thumbnail = await extractThumbnailBuffer(fullPath);

                    return {
                        preview: `data:image/jpeg;base64,${thumbnail.toString(
                            "base64"
                        )}`,
                        fullPath: fullPath,
                        fileName: fileName,
                ...
                const all = await Promise.all(videoPromises);
                return all;
            } catch (readError) {
                console.error("读取文件夹内容失败:", readError);
                result.error = "无法读取文件夹内容";
            }
        }
...
// 压缩视频
ipcMain.handle("compress-video", async (event, fullPath) => {
    try {
        const result = await compressVideo(fullPath);
        return result;
    } catch (error) {
